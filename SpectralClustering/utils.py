...
from ConsistentML import BatchSC, PCQ, ConsistentSC

methods = {
    'BatchSC': [(lambda k, s: BatchSC(n_components=k, seed=s), 'batch')], 
    'PCQ': [(partial(PCQ, alpha=alpha), 'alpha%.2f' % (alpha,)) for alpha in np.linspace(0, 1, 11)],
    'PCM': [(partial(ConsistentSC, lam=lam), 'lam%.8f' % (lam,)) for lam in np.append(0, np.logspace(-2, 1, 7))],
}
